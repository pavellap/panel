...
export default function(props) {
    ...
    const submit = () => {
        if (!(currentMinute >= 0 && currentMinute <= 59))
            ...
            ${hourHelper}:${minuteHelper}:00.000000`
        ...
        <WrapperModal>
                ...
                <div>
                    ...
                    <StyledSelect onChange={e => handleCurrentDay(e.target.value)}
                                  value={currentDay}>
                        {formArray(limits[currentMonth]).map((item, index )=> <MenuItem key={index} value={item}>{item + 1}</MenuItem>)}
                    </StyledSelect>
                </div>
                <div>
                    ...
                    <StyledSelect onChange={e => handleCurrentHour(e.target.value)}
                                  value={currentHour}>
                        {formArray(24).map((item, index) => <MenuItem key={index} value={item}>{item + 1}</MenuItem>)}
                    </StyledSelect>
                </div>
                <div>
